...
taxon = 'Harmonia axyridis'

wc = getData('worldclim', var='bio', res = 5, path='tmp/')
ext = extent(-125, -55, 20, 60)
wc = crop(wc, ext)
wc_df = raster::as.data.frame(wc, xy=TRUE) # for plotting
...
occ2thin = poThin(
  ...
  spacing = 100, #minimum distance between points in thinned data
...
pr_df = raster::as.data.frame(pr, xy=T)
...
est.loc = raster::extract(pr,  eval@occ.pts)
#extract model estimated suitability for background
est.bg = raster::extract(pr, eval@bg.pts)
...
pr_thr_df = raster::as.data.frame(pr_thr, xy=TRUE)
...
future_df = raster::as.data.frame(future, xy=TRUE) # for plotting
futurePreds = future[[predvars]]
names(futurePreds)=names(preds)
fuPr = predict(futurePreds, eval@models[[bestmod]], type = 'cloglog')
fuPr_df = raster::as.data.frame(fuPr, xy=T)

ggplot() +
  geom_raster(data = fuPr_df, aes(x = x, y = y, fill = layer)) +
  geom_point(data=sp_df, aes(x=longitude, y=latitude), col='red', cex=0.05) +
  coord_quickmap() +
  theme_bw() + 
  scale_fill_gradientn(colours=viridis::viridis(99),
...
fuPr_thr = fuPr>thr$sensitivity
fuPr_thr_df = raster::as.data.frame(fuPr_thr, xy=TRUE)

ggplot() +
  geom_raster(data = fuPr_thr_df, aes(x = x, y = y, fill = layer)) +
  geom_point(data=sp_df, aes(x=longitude, y=latitude), col='red', cex=0.2) +
  scale_fill_manual(values = c('black', 'blue')) +
  coord_quickmap() +
  theme_bw()
